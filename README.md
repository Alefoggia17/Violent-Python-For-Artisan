...
At the following link it is possible to extract a subset of intent/snippet pairs which will constitute the starting dataset: https://github.com/dessertlab/ESCAPE/tree/main/datasets/shellcode_ia32_extended.
Download only the `assembly-test.in` and `assembly-test.out` files. The first contains all the descriptions in natural language. The second contains all the Assembly language codes related to natural language descriptions.
